...
CoreAreaTriggerUnitElement.draw_links = function(self, t, dt, selected_unit, all_units)
	...
	for _,id in ipairs(self._hed.spawn_unit_elements) do
		...
		if self:_should_draw_link(selected_unit, unit) then
			self:_draw_link({from_unit = self._unit, to_unit = unit, r = 0.75, g = 0, b = 0})
		end
	end
	if self._hed.unit_ids then
		for _,id in ipairs(self._hed.unit_ids) do
			...
			if alive(unit) and self:_should_draw_link(selected_unit, unit) then
				self:_draw_link({from_unit = unit, to_unit = self._unit, r = 0, g = 0.5, b = 0.75})
...
CoreAreaTriggerUnitElement.add_element = function(self)
	...
	if ray and ray.unit then
		...
		if ray.unit:name():s() == "core/units/mission_elements/trigger_area/trigger_area" or string.find(ray.unit:name():s(), "point_shape", 1, true) then
			if not self._hed.use_shape_element_ids then
				self._hed.use_shape_element_ids = {}
			end
			local id = ray.unit:unit_data().unit_id
		...
		if string.find(ray.unit:name():s(), "data_instigator_rule", 1, true) then
			if not self._hed.rules_element_ids then
				self._hed.rules_element_ids = {}
			end
			local id = ray.unit:unit_data().unit_id
	...
	local ray = managers.editor:unit_by_raycast({mask = 1, ray_type = "body editor"})
	if ray and ray.unit then
		if not self._hed.unit_ids then
			self._hed.unit_ids = {}
		end
		local id = ray.unit:unit_data().unit_id
...
CoreAreaTriggerUnitElement.get_shape = function(self)
	...
	return (self._hed.shape_type ~= "box" or not self._shape) and self._hed.shape_type ~= "cylinder"
end
...
CoreAreaTriggerUnitElement._create_shapes = function(self)
	self._shape = CoreShapeManager.ShapeBoxMiddle:new({width = self._hed.width, depth = self._hed.depth, height = self._hed.height})
	self._shape:set_unit(self._unit)
	self._cylinder_shape = CoreShapeManager.ShapeCylinderMiddle:new({radius = self._hed.radius, height = self._hed.height})
...
CoreAreaTriggerUnitElement.create_values_ctrlrs = function(self, panel, panel_sizer, disable)
	...
	if not disable or not disable.trigger_type then
		self:_build_value_combobox(panel, panel_sizer, "trigger_on", {"on_enter", "on_exit", "both", "on_empty", "while_inside"})
	end
...
CoreAreaTriggerUnitElement._build_panel = function(self, panel, panel_sizer, disable_params)
	...
	if not panel_sizer then
		panel_sizer = self._panel_sizer
	end
...
CoreAreaTriggerUnitElement.scale_slider = function(self, panel, sizer, number_ctrlr_params, value, name)
	...
	slider:connect("EVT_SCROLL_CHANGED", callback(self, self, "set_size"), {ctrlr = slider, number_ctrlr_params = number_ctrlr_params, value = value})
	slider:connect("EVT_SCROLL_THUMBTRACK", callback(self, self, "set_size"), {ctrlr = slider, number_ctrlr_params = number_ctrlr_params, value = value})
	slider:connect("EVT_SCROLL_CHANGED", callback(self, self, "size_release"), {ctrlr = slider, number_ctrlr_params = number_ctrlr_params, value = value})
	slider:connect("EVT_SCROLL_THUMBRELEASE", callback(self, self, "size_release"), {ctrlr = slider, number_ctrlr_params = number_ctrlr_params, value = value})
...
if not CoreAreaOperatorUnitElement then
	CoreAreaOperatorUnitElement = class(MissionElement)
end
...
CoreAreaOperatorUnitElement.draw_links = function(self, t, dt, selected_unit, all_units)
	...
	for _,id in ipairs(self._hed.elements) do
		...
		if draw then
			self:_draw_link({from_unit = self._unit, to_unit = unit, r = 0.75, g = 0.75, b = 0.25})
		end
	end
end
...
CoreAreaOperatorUnitElement._build_panel = function(self, panel, panel_sizer)
	...
	CoreAreaTriggerUnitElement.create_values_ctrlrs(self, panel, panel_sizer, {trigger_type = true, instigator = true, amount = true})
	panel_sizer:add(EWS:StaticLine(panel, "", "LI_HORIZONTAL"), 0, 5, "EXPAND,TOP,BOTTOM")
	self:_build_value_combobox(panel, panel_sizer, "operation", {"none", "clear_inside"}, "Select an operation for the selected elements")
	for _,uses in ipairs(self._apply_on_checkboxes) do
...
if not CoreAreaReportTriggerUnitElement then
	CoreAreaReportTriggerUnitElement = class(CoreAreaTriggerUnitElement)
end
CoreAreaReportTriggerUnitElement.ON_EXECUTED_ALTERNATIVES = {"enter", "leave", "empty", "while_inside", "on_death", "rule_failed", "reached_amount"}
if not AreaReportTriggerUnitElement then
	AreaReportTriggerUnitElement = class(CoreAreaReportTriggerUnitElement)
end
