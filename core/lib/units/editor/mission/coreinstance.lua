...
function InstanceParamsUnitElement:_build_number(data, panel, sizer)
	...
	number:connect("EVT_COMMAND_TEXT_ENTER", callback(self, self, "_set_default_var_name"), {ctrlr = number, data = data})
	number:connect("EVT_KILL_FOCUS", callback(self, self, "_set_default_var_name"), {ctrlr = number, data = data})
end
function InstanceParamsUnitElement:_build_combobox(data, panel, sizer, options)
	...
	combobox:connect("EVT_COMMAND_COMBOBOX_SELECTED", callback(self, self, "_set_default_var_name"), {ctrlr = combobox, data = data})
	local toolbar = EWS:ToolBar(panel, "", "TB_FLAT,TB_NODIVIDER")
	toolbar:add_tool("ADD_UNIT_LIST", "Set from list", CoreEws.image_path("world_editor\\unit_by_name_list.png"), nil)
	toolbar:connect("ADD_UNIT_LIST", "EVT_COMMAND_MENU_SELECTED", callback(self, self, "_on_gui_select_name_dialog"), {combobox = params, data = data})
	toolbar:realize()
