...
function PortalLayer:build_panel(notebook)
	...
	top_spin:connect("EVT_SCROLL_THUMBTRACK", callback(self, self, "update_spin"), {spin = top_spin, value = "top"})
	top_spin:connect("EVT_COMMAND_TEXT_UPDATED", callback(self, self, "update_spin"), {spin = top_spin, value = "top"})
	self._ews_triggers.set_portal_top = callback(self, self, "set_height", {value = "top", spin = top_spin})
	spin_sizer:add(top_spin, 1, 0, "EXPAND")
	local bottom_spin = EWS:SpinCtrl(self._portal_panel, 0, "", "")
	bottom_spin:set_range(-500, 500)
	bottom_spin:connect("EVT_SCROLL_THUMBTRACK", callback(self, self, "update_spin"), {spin = bottom_spin, value = "bottom"})
	bottom_spin:connect("EVT_COMMAND_TEXT_UPDATED", callback(self, self, "update_spin"), {spin = bottom_spin, value = "bottom"})
	self._ews_triggers.set_portal_bottom = callback(self, self, "set_height", {value = "bottom", spin = bottom_spin})
	spin_sizer:add(bottom_spin, 1, 0, "EXPAND")
	portal_sizer:add(spin_sizer, 0, 0, "EXPAND")
	self._ctrlrs = {}
	self._ctrlrs.draw_base = draw_base
	self._ctrlrs.top_spin = {spin = top_spin, value = "top"}
	self._ctrlrs.bottom_spin = {spin = bottom_spin, value = "bottom"}
