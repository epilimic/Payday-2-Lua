...
function CoreParticleEditorVisualizers:create_billboard()
	...
	local defintensity = ""
	local p = CoreEffectProperty:new("intensity", "value_list", defintensity, [[
...
add_no_alpha - color becomes color of destination + billboard color]]
	...
	visualizer:add_property(p)
	help = [[
Determines method used to render billboard:
	Halo - no lighting]]
	...
	help = [[
Determines if a different age is assigned to each particle - this requires an AGE channel to be created.
...
scaled_channel - read from channel, scale with key curve]]
	...
	size_keys:add_key({t = 0, v = "100 100"})
	...
	size_scale_keys:add_key({t = 0, v = "1 1"})
...
scaled_channel - read from channel, scale with key curve]]
	...
	color_scale_keys:add_key({t = 0, v = "1 1 1"})
...
scaled_channel - read from channel, scale with key curve]]
	...
	opacity_keys:add_key({t = 0, v = "255"})
	...
	opacity_scale_keys:add_key({t = 0, v = "1"})
...
This parameter ONLY APPLIES for rotation_aligned billboards.]]
	...
	help = [[
Size of uv rectangle referenced by billboard.
...
function CoreParticleEditorVisualizers:create_trail()
	...
	local intensities = LightIntensityDB:list()
	local defintensity = ""
	p = CoreEffectProperty:new("intensity", "value_list", defintensity, [[
Intensity of billboards - choose a template that corresponds closest to the intensity range you want.
To scale the intensity over time, use the color.]])
	p:add_value("")
	for _, i in ipairs(intensities) do
...
scaled_channel - read from channel, scale with key curve]]
	...
	color_multiplier_keys:add_key({t = 0, v = "1 1 1"})
...
scaled_channel - read from channel, scale with key curve]]
	...
	opacity_multiplier_keys:add_key({t = 0, v = "1"})
...
scaled_channel - read from channel, scale over length with key curve]]
	...
	width_scale_keys:add_key({t = 0, v = "1"})
...
channel - read from channel]]
	...
	multiplier_keys:add_key({t = 0, v = "1.0"})
...
channel - read from channel]]
	...
	far_range_keys:add_key({t = 0, v = "100"})
