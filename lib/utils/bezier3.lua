...
local value = function(t, x1, x2, x3, x4)
	return (1 - t) ^ 3 * x1 + 3 * (1 - t) ^ 2 * t * x2 + 3 * (1 - t) * t ^ 2 * x3 + t ^ 3 * x4
end
local coefficients = function(x1, x2, x3, x4)
	return x4 - x1 + 3 * (x2 - x3), 3 * x1 - 6 * x2 + 3 * x3, 3 * (x2 - x1), x1
end
local value_for = function(t, a, b, c, d)
	return d + t * (c + t * (b + t * a))
end
local derivative1_for = function(t, a, b, c)
...
local to_bezier2 = function(x1, y1, x2, y2, x3, y3, x4, y4)
...
local split = function(t, x1, y1, x2, y2, x3, y3, x4, y4)
	local mt = 1 - t
...
local curve_collinearity_epsilon = 1.0E-30
...
function recursive_bezier(write, x1, y1, x2, y2, x3, y3, x4, y4, level, m_distance_tolerance2, m_angle_tolerance, m_cusp_limit)
	...
	if case == 0 then
		...
		if d2 > d3 then
			if m_distance_tolerance2 > d2 then
				write("line", x2, y2)
				return
			end
		elseif m_distance_tolerance2 > d3 then
			write("line", x3, y3)
			return
		end
	elseif case == 1 then
