...
function InventoryIconCreator:_create_weapon(factory_id, blueprint, weapon_skin)
	...
	if weapon_skin then
		...
		local cosmetics = {id = weapon_skin} or nil
	end
...
function InventoryIconCreator:start_all_weapons(test)
	...
	for _, factory_id in ipairs(weapons) do
		...
		table.insert(jobs, {factory_id = factory_id, blueprint = blueprint})
	end
...
function InventoryIconCreator:start_all_masks(with_blueprint)
	...
	for _, mask_id in ipairs(masks) do
		table.insert(jobs, {mask_id = mask_id, blueprint = blueprint})
	end
	self:start_jobs(jobs)
end
function InventoryIconCreator:start_one_mask(with_blueprint)
	...
	local blueprint = with_blueprint and self:_get_mask_blueprint_from_ui() or nil
	self:start_jobs({
		{mask_id = mask_id, blueprint = blueprint}
	})
end
...
function InventoryIconCreator:start_create()
	...
	self._old_data.base_chromatic_amount = managers.environment_controller:base_chromatic_amount()
...
function InventoryIconCreator:_create_weapons_page(notebook)
	...
	weapon_ctrlr:connect("EVT_COMMAND_COMBOBOX_SELECTED", callback(self, self, "_add_weapon_mods"), {panel = panel, sizer = comboboxes_sizer})
	...
	self:_add_weapon_mods({panel = panel, sizer = comboboxes_sizer})
	return panel
end
