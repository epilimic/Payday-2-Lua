...
function InventoryDescription.create_description_item(item, tweak, colors, ingame_format)
	...
	if item.bonus then
		...
		if bonus_data.exp_multiplier and bonus_data.money_multiplier then
			...
			bonus_string = bonus_string .. (not ingame_format and func_add_lb(ingame_format) or "") .. func_add_lb(ingame_format) .. managers.localization:text("steam_inventory_team_boost") .. " " .. managers.localization:text("steam_inventory_boost_xp_cash", {xp = xp_boost, cash = cash_boost})
