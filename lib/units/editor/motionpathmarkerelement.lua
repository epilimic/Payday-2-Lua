...
MotionpathMarkerUnitElement.PATH_TYPES = {"airborne", "ground"}
...
function MotionpathMarkerUnitElement:add_trigger(id, outcome, callback)
	self._triggers[id] = {outcome = outcome, callback = callback}
end
...
function MotionpathMarkerUnitElement:_build_panel(panel, panel_sizer)
	...
	self:_build_value_combobox(panel, panel_sizer, "motion_state", {"move", "wait"}, "Units on this marker will either wait or move through it.")
	local speed_params = {
		name = "Speed [km/h]:",
		panel = panel,
		sizer = panel_sizer,
