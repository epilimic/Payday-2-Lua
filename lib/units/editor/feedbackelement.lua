...
function FeedbackUnitElement:_build_panel(panel, panel_sizer)
	...
	self:_build_value_number(panel, panel_sizer, "range", {floats = 0, min = -1}, "The range the effect should be felt. 0 means that it will be felt everywhere")
	...
	self:_build_value_number(panel, camera_shaker_sizer, "camera_shake_amplitude", {floats = 2, min = -1}, "Amplitude basically decides the strenght of the shake", "amplitude")
	self:_build_value_number(panel, camera_shaker_sizer, "camera_shake_frequency", {floats = 2, min = -1}, "Changes the frequency of the shake", "frequency")
	self:_build_value_number(panel, camera_shaker_sizer, "camera_shake_attack", {floats = 2, min = -1}, "Time to reach maximum shake", "attack")
	self:_build_value_number(panel, camera_shaker_sizer, "camera_shake_sustain", {floats = 2, min = -1}, "Time to sustain maximum shake", "sustain")
	self:_build_value_number(panel, camera_shaker_sizer, "camera_shake_decay", {floats = 2, min = -1}, "Time to decay from maximum shake to zero", "decay")
	...
	self:_build_value_number(panel, rumble_sizer, "rumble_peak", {floats = 2, min = -1}, "A value to determine the strength of the rumble", "peak")
	self:_build_value_number(panel, rumble_sizer, "rumble_attack", {floats = 2, min = -1}, "Time to reach maximum rumble", "attack")
	self:_build_value_number(panel, rumble_sizer, "rumble_sustain", {floats = 2, min = -1}, "Time to sustain maximum rumble", "sustain")
	self:_build_value_number(panel, rumble_sizer, "rumble_release", {floats = 2, min = -1}, "Time to decay from maximum rumble to zero", "release")
