	...
	while not self._end_of_curved_path do
		...
		if complete then
			...
			elseif self._next_is_nav_link then
				...
				if not self._sync or self._next_is_nav_link or not s_path[3] or not self:_reserve_nav_pos(next_pos, self._nav_point_pos(s_path[3]), self._nav_point_pos(c_path[#c_path]), vel) then
				end
...
function CopActionWalk:get_walk_to_pos()
	return self._nav_point_pos((not self._simplified_path or not self._simplified_path[2]) and self._nav_path and self._nav_path[2])
end
