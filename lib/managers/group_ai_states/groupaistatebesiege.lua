...
function GroupAIStateBesiege:_check_spawn_phalanx()
	if not Global.game_settings.single_player and self._phalanx_center_pos and self._task_data and self._task_data.assault.active then
		if self._task_data.assault.phase == "build" or self._task_data.assault.phase == "sustain" then
			if not self._phalanx_spawn_group then
				...
				if not self._phalanx_despawn_time or now >= self._phalanx_despawn_time + respawn_delay then
					...
					print("Phalanx won't spawn: Has despawned recently!")
				end
			else
				print("Phalanx won't spawn: Spawn group still present!")
			end
		else
			print("Phalanx won't spawn: Assault not in phase 'build' or 'sustain'!")
			do break end
			break
		end
	else
