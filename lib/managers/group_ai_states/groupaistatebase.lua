...
function GroupAIStateBase:fill_criminal_team_with_AI(is_drop_in)
	while true do
		...
		elseif not (managers.criminals:nr_taken_criminals() < CriminalsManager.MAX_NR_CRIMINALS) or not (managers.criminals:nr_AI_criminals() < managers.criminals.MAX_NR_TEAM_AI) or not self:spawn_one_teamAI(is_drop_in) then
			break
		end
	end
end
