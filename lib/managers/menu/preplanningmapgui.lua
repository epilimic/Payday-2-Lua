...
function PrePlanningPoint:select_mouse_over()
	if self._active_node.current_type or self._active_node.current_plan then
...
function PrePlanningPoint:mouse_moved(x, y, used)
	...
	local is_in_menu = (not self._active_node.current_type or not self._properties.type[self._active_node.current_type]) and self._active_node.current_plan and self._properties.category[self._active_node.current_category]
...
function PrePlanningCustomPoint:init(map_panel, data, texture_width, texture_height, location_rotation, active_node, name)
	...
	self._x = ax
	self._y = ay
	self._panel = self._map_panel:panel({alpha = 0.9, layer = 4})
	self._panel:set_center(self._map_panel:w() * self._x, self._map_panel:h() * self._y)
...
function PrePlanningCustomPoint:_update_state()
	local selected = not self._mouse_selected and self._active_node.current_custom and self._active_node.node:selected_item() and self._active_node.node:selected_item():name() == self._name
...
local seconds_per_draw = 0.016666668
function PrePlanningMapGui:update_drawing(t, dt)
	if self._draw_mode then
		...
		if time_diff > seconds_per_draw then
			self._last_draw_t = t
...
function PrePlanningMapGui:create_text_button(params)
	...
	local text = not params.text and params.text_id and managers.localization:text(params.text_id)
	...
	if self._released_map or not self._grabbed_map then
		...
		if self._lerp_map then
			...
			if dx ~= 0 or dy ~= 0 then
				...
				if self._lerp_map.clbk and 1 > math.abs(mx) and 1 > math.abs(my) then
					self._lerp_map.x = self._map_x
					self._lerp_map.y = self._map_y
				end
			else
