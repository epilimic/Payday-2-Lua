...
function SkillTreePage:init(tree, data, parent_panel, fullscreen_panel, tree_tab_h, skill_prerequisites)
	...
	local panel_h = 0
	local h = (parent_panel:h() - tree_tab_h - 60) / (8 - CONSOLE_PAGE_ADJUSTMENT)
	for i = 1, 7 do
...
function SkillTreeGui:_setup()
	...
	if #specialization_data > 0 then
		...
		local h = (self._specialization_panel:h() - skill_tree_h - 60) / 8 * 7 + 10
		...
		local spec_description_panel = desc_box_panel:panel({valign = "grow", halign = "grow"})
...
function SkillTreeGui:set_spec_scroll_indicators(tree)
	...
	elseif self._active_spec_tree - NUM_TREES_PER_PAGE + 1 > self._specialization_scroll_y then
	...
	self._spec_scroll_bar_panel:set_visible(#self._spec_tree_items > NUM_TREES_PER_PAGE)
end
...
function SkillTreeGui:_set_selected_spec_item(item, no_sound)
	if self._selected_spec_item ~= item then
		...
		if item then
			no_sound = item.tree and no_sound and self._active_spec_tree == item:tree()
			if item.tier then
...
function SkillTreeGui:move_spec_item(x, y)
	...
	if self._selected_spec_tier then
		current_tree = self._selected_spec_item and self._selected_spec_item:tree() or current_tree
...
function SpecializationTreeItem:init(tree, parent_panel, tab_iem)
	...
	if tree >= num_trees_per_page then
		tree_panel:move(0, -1)
	end
	...
	active_text:set_bottom(tree_panel:top())
	...
	selected_text:set_bottom(tree_panel:top())
...
function SpecializationTreeItem:refresh()
	...
	selected_text:set_alpha(not self._active and self._selected and 1 or 0)
...
function SpecializationTierItem:init(tier_data, tree_panel, tree, tier, x, y, w, h)
	...
	self._selected_size = {w, h}
	local texture, texture_rect = "guis/textures/pd2/specialization/perk_icon_card", {
		0,
		0,
		64,
		92
...
function SkillTreeGui:_update_rename_skill_switch()
	...
	if self._renaming_skill_switch then
		...
		self._rename_caret:set_world_position(skill_set_text:right(), skill_set_text:top())
	else
